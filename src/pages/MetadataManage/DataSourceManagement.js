...
class DataSourceManagement extends PureComponent {
  ...
  componentDidMount() {
    ...
    //判断是不是从详情页跳转的
    if (
      location.hasOwnProperty('params') &&
      location['params'].hasOwnProperty('name') &&
        ...
        form.validateFieldsAndScroll((err, values) => {
          if (!err) {
            let params = {
              ...
              type:
                values.databaseType !== 'ALL'
                  ? T.lodash.isUndefined(values.databaseType)
                    ? ''
                    : values.databaseType
                  : '',
              status:
                values.status !== 'ALL'
                  ? T.lodash.isUndefined(values.status)
                    ? ''
                    : values.status
                  : '',
              page: currentPage,
  ...
  renderSelectOption = (selectDataSource, selectType) => {
    if (selectType === 'databaseType') {
      return selectDataSource.map(item => {
        return (
        ...
        <Row className={styles.dataSourceTitle} gutter={24}>
          ...
          <Form layout="inline" onSubmit={this.searchDataSource}>
            <Col xl={5} lg={5} md={5} sm={24}>
              <FormItem
                label={
                  <FormattedMessage id="metadataManage.dataSourceManagement.dataSourceSearchForm.dataSourceName.label" />
                }
              >
                {getFieldDecorator('dataSourceName', {
                  ...
                  <Input
                    ...
                    placeholder={formatMessage({
                      id:
                        'metadataManage.dataSourceManagement.dataSourceSearchForm.dataSourceName.placeholder',
                    })}
                  />
                )}
              </FormItem>
            ...
            <Col xl={5} lg={5} md={5} sm={24}>
              <FormItem
                label={
                  <FormattedMessage id="metadataManage.dataSourceManagement.dataSourceSearchForm.databaseType.label" />
                }
              >
                {getFieldDecorator('databaseType', {
                  ...
                  <Select
                    ...
                    placeholder={formatMessage({
                      id:
                        'metadataManage.dataSourceManagement.dataSourceSearchForm.databaseType.placeholder',
                    })}
                  >
                    {/*dataSourceTypeTreeData*/}
                    {/*<Option value="" key="">
            ...
            <Col xl={5} lg={5} md={5} sm={24}>
              <FormItem
                label={
                  <FormattedMessage id="metadataManage.dataSourceManagement.dataSourceSearchForm.status.label" />
                }
              >
